...
class FormatParser:
    ...
    def parse_kv(body_str):
        ...
        for line_num, line in enumerate(body_str.strip().split('\n'), 1):
            if line.strip():
                record = {}
                # 支持 key=value 和 key="value with spaces" 格式
                pairs = re.findall(r'(\w+)=("(?:[^"\\]|\\.)*"|\S+)', line)
                if not pairs:
                    raise ValueError(f"KV 格式解析失败 (第 {line_num} 行): 未找到有效的 key=value 对")
...
class HTTPSinkHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if is_gzip:
                try:
                    ...
                    print(f"   压缩率: {(1 - content_length/len(decompressed))*100:.1f}%")
                    print(f"-" * 70)
            ...
            self.send_json_response(400, {
                "status": "error",
                "message": f"{fmt.upper()} 格式解析失败: {str(e)}"
            })
        except Exception as e:
            print(f"❌ 未知错误: {e}")
            import traceback
    ...
    def do_POST(self):
        ...
        # /gzip/ingest/{format}
        if len(path_parts) == 3 and path_parts[0] == 'gzip' and path_parts[1] == 'ingest':
            fmt = path_parts[2]
            self.handle_ingest(fmt, require_auth=False, is_gzip=True)
            return
        
        # /auth/ingest/{format}
        if len(path_parts) == 3 and path_parts[0] == 'auth' and path_parts[1] == 'ingest':
            fmt = path_parts[2]
