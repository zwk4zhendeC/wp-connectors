//! Doris 并发异步发送测试程序
//!
//! 用于测试多个异步任务并发发送数据到 Doris
//!
//! 运行方式：
//! ```bash
//! cargo run --example doris_concurrent_test --features doris
//! ```
...
async fn main() {
    ...
    if total_sent > 0 {
        ...
        println!(
            "   SELECT COUNT(*) FROM test_db.wp_nginx WHERE wp_src_key LIKE 'concurrent_test_%';"
        );
